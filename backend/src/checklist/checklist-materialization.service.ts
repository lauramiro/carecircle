...
export class ChecklistMaterializationService {
  ...
  async materializeForMedication(
    medicationId: string,
    reason: string,
  ): Promise<void> {
    const med = await this.medicationRepo.findById(medicationId);
    if (!med || med.status !== 'active' || med.schedule_type === 'as_needed')
      return;
    ...
    if (!groupCtx?.patientId) {
      this.logger.warn(
        `materialize_skip no_group medicationId=${medicationId} reason=${reason}`,
      );
      return;
    }
    ...
    const cursorAt = med.materialization_cursor_at
      ...
      : null;
    const now = new Date();
    const allSlots = enumerateFutureDoseSlots(
      slotMed,
      groupCtx.preferredTimezone,
      now,
      cursorAt,
    );
    ...
    const futureDue =
      await this.checklistRepo.countFutureDueItems(medicationId);
  ...
  async extendPendingSchedules(limit: number): Promise<void> {
    const schedules = await this.checklistRepo.findPendingSchedulesDue(
      24,
      limit,
    );
    for (const schedule of schedules) {
      try {
        await this.materializeForMedication(
          schedule.medication_id,
          'cron_schedule',
        );
      } catch (err) {
        this.logger.warn(
          `schedule_extend_failed medicationId=${schedule.medication_id}`,
          err,
        );
        await this.checklistRepo.upsertSchedule({
          medicationId: schedule.medication_id,
          nextComputeAt: new Date(
...
export class ChecklistReconciliationService {
  ...
  async reconcileAfterMedicationEdit(
    ...
    await this.alertRepo.cancelOpenAlertsForMedication(
      newMed.id,
      'schedule_changed',
    );
