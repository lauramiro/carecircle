-- medications.specific_times is "time without time zone[]", but
-- medication_daily_dose_count() was declared with p_specific_times text[],
-- so the call from reset_low_stock_alert_when_restocked() (which passes
-- new.specific_times directly) fails on every medications insert/update
-- with "function ... does not exist" — there is no implicit time[] -> text[]
-- cast. Recreate with the matching parameter type.
