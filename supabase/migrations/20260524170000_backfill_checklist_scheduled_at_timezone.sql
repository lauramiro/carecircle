-- Backfill checklist_items.scheduled_at from local checklist_date + scheduled_time
...
SET
  scheduled_at = (
    (to_char(dmc.checklist_date, 'YYYY-MM-DD') || ' ' || ci.scheduled_time || ':00')::timestamp
...
WHERE ci.checklist_id = dmc.id
  ...
  AND ci.scheduled_time IS NOT NULL
