-- Fix CC-165: update_invite_status raised "column reference 'group_id' is
-- ambiguous" (42702) on every real call. RETURNS TABLE(group_id uuid) creates
-- a plpgsql OUT variable named group_id; under the default
-- #variable_conflict = error, `select * into v_invite from public.invites`
-- can't tell whether group_id refers to the OUT variable or the invites
-- column. use_column tells it to prefer the table column, which is what the
-- function actually wants.
